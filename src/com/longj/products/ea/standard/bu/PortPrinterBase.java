...
public class PortPrinterBase {
	...
	public static void main(String[] arg0){
		...
		String nreew = Integer.toHexString(a);
		System.out.println(nreew);
		String aaa = toStringHex(nreew);
	...
	public PortPrinterBase(OutputStream out, String printType) {
		...
		initPrinter();
		// String lineCountStr =
		// PrinterParameterConf.printerParameterConf.getProperty(PrinterParameterConf.LINEPRINTERCHARCOUNT_NAME);
		try {
	...
	/*
	 * 切纸指令GS V m m 0,48 Executes a full cut(cuts the paper completely) 1,49
	 * Excutes a partilal cut(one point left uncut)
	 */
	...
	// 如果返回结果为22
	/**
	 * 、DLE EOT n 实时状态传送 [格式] ASCII码 DLE EOT n 十六进制码 10 04 n 十进制码 16 4 n [范围] 1
	 * ≤ n ≤ 4 [描述] 根据下列参数，实时传送打印机状态，参数 n 用来指定所要传送的打印机状态： n = 1：传送打印机状态 n =
	 * 2：传送脱机状态 n = 3：传送错误状态 n = 4：传送纸传感器状态 [注释] 打印机收到该命令后立即返回相关状态
	 * 该命令尽量不要插在2个或更多字节的命令序列中。 即使打印机被ESC =(选择外设)命令设置为禁止，该命令依然有效。
	 * 打印机传送当前状态，每一状态用1个字节数据表示。 打印机传送状态时并不确认主机是否收到。 打印机收到该命令立即执行。
	 ...
	 *            需打印的字符串
	 * @param align
	 *            打印的位置 LEFT/CENTER/RIGHT 其他为默认居左打印
	 */
